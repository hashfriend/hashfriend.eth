...
- On 23 September at 15:30 UTC, the team [replied](https://t.me/Singularity_Fi/265509) to my [11:33 UTC message](https://t.me/Singularity_Fi/265508), which repeated the four questions and asked why an EOA governs the gold vault. It confirmed fee tier 42 as part of the failure that enabled the exploit but not the sole cause, because the oracle "failed to handle the missing price correctly". It said the tier was used to prevent certain Uniswap pools from determining vault asset values, that this protection proved incomplete, and that the latest fixes address it. The post-mortem is in internal review with no date and will include the recovery plan, whose logistics it said were in internal testing. It named no funding source. It said its vaults use multisig governance, that the gold vault and a few other BNB vaults were opened for external non-whitelisted testing and are therefore EOA-controlled, and that users should wait until they are public and multisig-governed.
- As of 23 September, the promised post-mortem remains unpublished and the team has published no reimbursement terms. The 23 September reply answered whether tier 42 contributed and gave the reason it was set. Who chose it, the publication date and whether reimbursement comes from project funds or collected fees remain unanswered.
...
- The [21 September Base snapshot](evidence/2026-09-21-base-snapshot.json), at block 51,603,821, confirms the governance Safe still had three owners and a threshold of two. Both Base Safes retained the same owners, thresholds and nonces as on 8 September. The admin still held the guardian role on every Base production manager and both admin roles on the replacement Uniswap oracle. No Base vault's governance, guardian, management, permission, shutdown, fee or reference-oracle settings changed between those snapshots.
- The [8 September snapshot](evidence/2026-09-08-original.jsonl) and the 21 September [Base](evidence/2026-09-21-base-snapshot.json) and [other-chain](evidence/2026-09-21-other-chains.json) snapshots show the same governance split. The Safe governs all nine Base vaults, dynBnbUSDC and dynBnbBTCB. The admin account directly governs the other eight production vaults: dynMainUSDC, dynMainUSDCv2, dynMainUSDCv3, dynArbETH, dynArbBTC, dynBnbUSDT, dynBnbBNB and dynBnbGold. The last three have permissions disabled. The snapshots do not establish when each governance account was assigned.
...
- The team's 23 September reply said its vaults use multisig governance and attributed EOA control to BNB vaults opened for testing. The 21 September snapshots also show the admin account directly governing the five permissioned Ethereum and Arbitrum vaults and holding the guardian role on every Base production manager.
...
- On 15 September, five successful admin transactions configured cbBTC/SFI swap routes through WETH on Base. UniswapV3DynaRouter received default fees of 500 for cbBTC/WETH and 10,000 for WETH/SFI; DynaRouterRegistry received the router and forward/reverse pool paths. The [receipts, decoded inputs and pool/token reads](evidence/2026-09-21-base-history.json) identify the SFI token as `0x863B7364a29daA23C9FCBD02e27D129A8B185891`. These are swap-execution settings. Actual vault purchases require separate trade evidence.
- On 16 September, the same admin opened dynBnbBNB with a successful [`setPermissionDisabled(true)` transaction](https://bscscan.com/tx/0x3706c9a124d57e2f853b5ba08529f69d4552eef9a0666292a512e7d99388732b), at BNB block 122,219,805. The [receipt and adjacent-block reads](evidence/2026-09-16-bnb-bnb-opening.json) confirm the permission flag changed from false to true. All five registered BNB production vaults had permissions disabled at the 21 September snapshot; this opening did not increase the vault count.
- On 22 September at 19:36 UTC, the team [announced the gold vault](https://t.me/Singularity_Fi/265496) as in beta/preview mode and coming soon to the community. The [23 September read](evidence/2026-09-23-gold-state.json), at BNB block 123,555,598, found dynBnbGold's `permissionDisabled` still true. For an address without `PERMITTED_USER`, `maxDeposit` returned `type(uint192).max` minus `totalAssets`. The admin account was both governance and guardian of its manager and had no contract code. Assets rose from 0.010999 XAUt at the 21 September snapshot to 0.242323 XAUt. The depositors are unidentified, because the public BNB RPC refuses historical log queries without a token and `SFI_3XPL_TOKEN` was unavailable.
- The registered production total rose from 18 vaults on 31 August to 19 by 8 September and remained 19 on 21 September: nine on Base, three on Ethereum, two on Arbitrum and five on BNB Chain, with none on Optimism or Polygon. The [Base snapshot](evidence/2026-09-21-base-snapshot.json) and [other five chains](evidence/2026-09-21-other-chains.json) contain pinned blocks and per-vault settings. Non-Base reads reuse compatible ABI fragments from the Base source cache; they do not establish matching deployed source builds.
...
The [complete Base snapshot](evidence/2026-09-21-base-snapshot.json) at block 51,603,821 covers all nine production vaults, both Safes and all three current oracle contracts through their deployed ABIs. The block hash is `0x332f484952776e39a28959c3b5f251227cb086eeb5a76167288c63c75d36d116`. The drained vault reported 102.428963 USDC, compared with 102.063648 USDC at the 8 September snapshot. Its recorded reserve balances were unchanged. The replacement vault reported 1,170.496275 USDC and remained permissioned. These reads do not establish whether reimbursements occurred through other accounts. Safe proxies resolve through the implementation address in storage slot zero.
...
The old oracle's `getPrice(base, quote, amount)` substitutes tier 3000 whenever `uniV3fee` returns 0, for the direct route and both legs of the WETH fallback. An unset fee therefore could not disable a route, while tier 42 made the direct lookup return no pool. The [pre-drain fee reads](evidence/2026-04-25-weth-route-fees.json) at block 45,183,966 show all six token/WETH routes unset, resolving to 3000, the tier of the attacker's pools; WETH/USDC was set to 100. With the threshold at zero, `liquidity() < minLiquidityThreshold` could never reject a pool. This matches the team's 23 September description of tier 42 as an incomplete protection against certain pools.
...
On 6 September, the payout address [transferred 1,347.499541 ysUSDC](https://basescan.org/tx/0xd4b2d3509933800635f9d72e517e87edc73b0716cb735167fffd0ca37d9f9a5a) to `0x396134838Da76b63d797C5f32D0bC35A49e8ac1A`. The token and raw amount exactly match the residual payout in drain receipt log 594. The [movement receipts and destination reads](evidence/2026-09-06-residual-movements.json) confirm that address still held the full amount at Base block 51,604,100 on 21 September. `convertToAssets()` and `previewRedeem()` quoted 1,422.793535 USDC at both the transfer block and the current read, while `maxRedeem()` for the recipient returned zero. This establishes a transfer of residual tokens; the quoted value does not establish realized proceeds or reimbursement. The [latest indexed histories of the known exploit addresses](evidence/2026-09-21-attacker-base-history.json) are bounded Base evidence, not a complete cross-chain fund-flow trace.
...
The [replacement-oracle reads](evidence/2026-07-19-replacement-oracle-fees.json) check those 60 pairs at block 48,858,126 on 19 July. Three had configured routes: USDT/USDC and WETH/USDC at fee 100, and cbBTC/USDC at fee 500. The factory returned tick spacings of one and ten for those tiers. The other 57 pairs were unset in the replacement Uniswap oracle. A [21 September recheck](evidence/2026-09-21-base-oracle-and-fees.json), at block 51,603,861, found all 60 fee settings unchanged. This is a check of the same pairs, not an enumeration of every possible route.

The cached replacement UniswapV3Oracle source, matched at block 51,038,969, returns `(0, 0)` for a route without a fee instead of substituting 3000. It rejects a pool whose current or harmonic-mean liquidity is at or below a per-pair threshold, so a pool without liquidity fails even at threshold zero. This supports the team's 23 September statement that the latest fixes address the fee-tier protection. The replacement ReferenceAssetOracle's `ReferenceAssetOracle.sol` is identical to the pre-drain source and still accepts any result with a nonzero timestamp, including a zero price. These are source comparisons; the per-pair thresholds on the three configured routes were not read.
...
The [21 September fee-Safe reads](evidence/2026-09-21-base-oracle-and-fees.json), at block 51,603,861, value its drained-vault shares at 1.448686 USDC, compared with 1.364935 USDC in the [8 September reads](evidence/2026-01-16_to_2026-09-08-fees.jsonl). The current Base snapshot confirms four owners, threshold two and the drained vault's enabled fee settings. On 16 January, an owner addition and removal in blocks 40,889,668 and 40,889,696 had left three owners. The share valuation does not establish funds available for reimbursement elsewhere.

Fee allocation continued on 21 September. A successful [`reportReserve()` transaction](https://basescan.org/tx/0xa6344b703c24669df67be3536fbe2de718db699d8525e63deb8123d4d42a3090), in block 51,600,155, minted 98.149293853536496189 shares to the drained vault and transferred them to the fee Safe. The [RPC receipt and logs](evidence/2026-09-21-base-history.json) establish that allocation. Together with three earlier indexed allocations on 11, 15 and 17 September, it accounts for the Safe's balance increase of 333.966529199130083919 shares since the prior snapshot. Blockscout's address history omitted the 21 September allocation; the RPC check recovered it.
