...
Redirects in `astro.config.mjs` generate HTML meta refresh pages for static IPFS hosting.
...
`src/pages/og/[slug].png.ts` builds one Open Graph card per page from the same frontmatter — satori lays it out, resvg writes the PNG. It uses the vendored `src/assets/avatar.png` and the `@fontsource/geist*` WOFFs, which exist only because satori needs raw font bytes; the pages themselves get Geist from Astro's Google font provider. Keep them at `.woff` — satori cannot read WOFF2.
